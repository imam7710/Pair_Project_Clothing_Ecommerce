...
func (r *productRepo) GetAll() ([]domain.Product, error) {
	rows, err := r.db.Query(`SELECT id, category_id, sku, name, size, color, price, stock FROM products ORDER BY id ASC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.ID, &p.CategoryID, &p.SKU, &p.Name, &p.Size, &p.Color, &p.Price, &p.Stock); err != nil {
			return nil, err
		}
...
func (r *productRepo) Create(p domain.Product) error {
	_, err := r.db.Exec(`INSERT INTO products (category_id, sku, name, description, size, color, price, stock) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
