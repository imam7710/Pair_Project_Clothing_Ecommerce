...
func (r *reportRepo) GetUserReports() ([]domain.UserReportDTO, error) {
	rows, err := r.db.Query(`SELECT u.id, u.email, u.role, COALESCE(p.full_name, '-'), COALESCE(p.phone, '-') FROM users u LEFT JOIN profiles p ON u.id = p.user_id ORDER BY u.id ASC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *reportRepo) GetStockReports() ([]domain.StockReportDTO, error) {
	rows, err := r.db.Query(`SELECT sku, name, size, color, stock, CASE WHEN stock = 0 THEN 'HABIS' WHEN stock < 5 THEN 'MENIPIS' ELSE 'AMAN' END FROM products ORDER BY stock ASC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *reportRepo) GetOrderReports() ([]domain.OrderReportDTO, error) {
	rows, err := r.db.Query(`SELECT o.id, u.email, o.total_amount, o.status, DATE_FORMAT(o.order_date, '%Y-%m-%d %H:%i') FROM orders o JOIN users u ON o.user_id = u.id ORDER BY o.order_date DESC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		rows.Scan(&d.OrderID, &d.Pembeli, &d.TotalAmount, &d.Status, &d.WaktuTransaksi)
		list = append(list, d)
	}
	return list, nil
}
